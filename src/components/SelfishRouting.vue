<template>
  <div id="selfish-routing">
    <v-card-title primary-title>
        <h3 class="headline mb-0">Selfish routing</h3>
    </v-card-title>
    <v-img>
      <div class="selfish-routing" style="text-align: center">
      </div>
    </v-img>
    ...
    <v-card-text class="headline font-weight-bold">
      ...
      <p>Avg. estimated cost: {{ getTotalCost() | round }}</p>
      <p v-if="(flow_link1 === 1.0) && (flow_link2 === 0.0)">User equilibrium (Wardrop equilibrium under users' selfish criteria)</p>
      <p v-if="(flow_link1 === 0.5) && (flow_link2 === 0.5)">Social optimum (Wardrop equilibrium under users' altruistic criteria)</p>
    </v-card-text>
  </div>
</template>
...
export default {
  ...
  methods: {
    getFlowLink1 () {
      ...
      d3.selectAll('.selfish-routing svg').remove()
      // d3.selectAll('.selfish-routing svg g circle').remove()
      var svg = d3.selectAll('.selfish-routing').append('svg')
      var g = svg.append('g')
      ...
      g.selectAll('circle')
        ...
        .attr('stroke', 'black')
        .attr('stroke-width', '3px')
      return Math.round(this.flow_link1 * 100) / 100
    },
    getFlowLink2 () {
      const getLabel = (i) => {
        if (i === 1) {
      ...
      return Math.round(this.flow_link2 * 100) / 100
    },
    getTotalCost () {
  ...
  mounted () {
    ...
    var svg = d3.select('.selfish-routing').append('svg')
    var g = svg.append('g')
    var lineGenerator = d3.line().curve(d3.curveCardinal)
